#!/bin/bash
# Script for checking the current active power of a Tasmota smartplug.
...
# --- Power Retrieval ---
POWER=$(get_active_power "$TARGET_IP")

# --- Output and Exit ---
# Check if the result is a valid number.
if [[ "$POWER" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
  echo "$POWER"
  LOG_INFO "Current active power of device at $TARGET_IP is: $POWER W"
  exit 0
else
  # Log error to stderr
  LOG_FATAL "Could not retrieve power data from $TARGET_IP."
fi