#!/bin/bash
# Script for checking yesterday's energy consumption of a Tasmota smartplug.
...
# --- Energy Retrieval ---
ENERGY=$(get_energy_yesterday "$TARGET_IP")

# --- Output and Exit ---
# Check if the result is a valid number.
if [[ "$ENERGY" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
  echo "$ENERGY"
  LOG_INFO "Yesterday's energy consumption of device at $TARGET_IP is: $ENERGY Wh"
  exit 0
else
  # Log error to stderr
