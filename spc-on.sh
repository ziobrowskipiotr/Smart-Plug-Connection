#!/bin/bash
# Script using for turning smartplug with tasmota firmware on
...
if [[ -n "$NAME" && -z "$IP" ]]; then
  IP=$(sqlite3 "$DB_FILE" "SELECT ipv4 FROM devices WHERE name = '$NAME' LIMIT 1;")
  if [[ -z "$IP" ]]; then
...
if [[ -n "$NAME" && -n "$IP" ]]; then
  DB_IP=$(sqlite3 "$DB_FILE" "SELECT ipv4 FROM devices WHERE name = '$NAME' LIMIT 1;")
